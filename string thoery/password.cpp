/*Rabin karp +binary search on length 
...
* from TLE. However there must be some faster way to do that. I you could
...
int main( int argc , char ** argv )
{
	...
	fr(i,1, n-2){
		ll pref = (hashPre[i-1])%MOD;
		ll suff = ((((MOD+hashPre[n-1]-hashPre[n-i-1])%MOD)%MOD)*inv[n-i])%MOD;//like prefix function stores all length where prefix==suffix
		if(pref==suff)possible.push_back(i);
	}
// 	for(auto x:possible)
// 	cout<<x<<" ";

	sort(all(possible));//bianry searching on th elenth and checking if in middle also any infix==pref ifyes we store it and take the max possible 
	...
	while(lo<=hi && possible.size()>0){
		...
		bool ok = 0;
		fr(i, 1, n-leng-1){
			ll inFix = (((hashPre[i+leng-1]-hashPre[i-1])%MOD)*inv[i])%MOD;
			ll pref = (hashPre[leng-1])%MOD;
			if(inFix==pref){
				ok=1;
				break;
			}
